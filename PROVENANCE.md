...
| **Polish pass: stub intent expansion** (`backend/app/llm_provider.py` `StubProvider`, `_question_lead`, `_stub_constraints`) — deterministic keyword handling for the common chat intents with no key: workload/units/why-this-day/requirement-coverage questions, lighter/avoid-day/no-early/no-late/drop-course/topic-drop modifications, and interest asks. Still only a provider: its output flows through the same solver and verifier gate as any model's; it gained no new authority | agent-generated | Octavius | pending review |
| Polish pass: grounding facts enrichment (`backend/app/orchestrator.py` `ScheduleContext.requirements_advanced`, `build_chat_messages` facts payload) — per-section meeting times and requirement-group names added to the `<facts>` grounding every provider receives. Prompt construction only: the verifier gate, `constraints_to_solver_inputs`, and `orchestrate_chat_turn`'s gating logic are untouched (correctness-critical rows above remain accurate) | agent-generated | Octavius | pending review |
| Polish pass: provider-failure fallback (`backend/app/main.py` `/chat`) — a `ProviderError` (missing config, transport failure, or model output that fails `ChatTurn` schema validation) no longer 500s: the turn returns a fallback reply with zero claims, unchanged constraints, and the calendar as it was. Malformed output is treated like a failed verification — nothing unverified is ever shown | agent-generated | Octavius | pending review |
| Polish pass: stub-intent + malformed-output tests (`backend/tests/test_llm_provider.py`, `backend/tests/test_api.py`) — each stub intent asserted offline (sockets blocked) and through the real solver+verifier at the API; malformed JSON, unknown claim types, and missing provider config all fall back gracefully with no claim leak. Suite green before (127) and after (152), no secrets | agent-generated | Octavius | pending review |
| Polish pass: UI/UX (`frontend/src` — `App.tsx` stepper, `WeekGrid.tsx` palette/legend/hover-focus detail, `SchedulePanel.tsx` notices + fewer-than-K message + disclaimer, `ConfirmationPanel.tsx` per-prereq re-solve feedback, `ChatThread.tsx` turn-outcome tags + live-region thread, `PrereqChecklist.tsx` progress count, `RationalePanel.tsx` grouping, `styles.css` token system + focus styles) — presentation only; no change to the data flow, endpoints, or what is/isn't verified. Typechecked and production-built | agent-generated | Octavius | pending review |
| Architecture doc + diagram refresh (`docs/architecture.md`, `docs/architecture.svg`) — narrative updated for the polish pass (grounding facts, provider-failure fallback, stub intent routing, 127 → 152 tests); the SVG redrawn as the **as-built** system matching the doc's Mermaid, replacing Wendy's *target*-design sketch (row above; original preserved at `1f4b408`) | agent-generated | Octavius | pending review |
